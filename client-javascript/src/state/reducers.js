...
import { STATUS_OK, STATUS_ERROR
       , AUTHENTICATE, LOGOUT
       , HISTORY_FETCH, HISTORY_CANCEL_EDIT_ENTRY, HISTORY_EDIT_ENTRY, HISTORY_INVALIDATE
       , SAVE_TIME_DISTANCE
       , SAVE_WEIGHT 
       } from './actions'
    ...
    switch (action.type) {
        case AUTHENTICATE:
            switch (action.status) {
                case STATUS_OK:
                    return Object.assign({}, state, { isFetching: false, error: null, creds: action.value })
                case STATUS_ERROR:
                    return Object.assign({}, state, { isFetching: false, error: action.error, creds: null })
                default:
                    return Object.assign({}, state, { isFetching: true, error: null, creds: null })
            }
        case LOGOUT:
            switch (action.status) {
                case STATUS_OK:
                    return Object.assign({}, state, { isFetching: false, error: null, creds: null })
                default:
                    return state
            }
        default:
...
const handleHistoryActions = (state, action) => {
    switch (action.type) {
        ...
        case HISTORY_CANCEL_EDIT_ENTRY:
            return Object.assign({}, state, { currentlyEditing: null })
        case HISTORY_EDIT_ENTRY:
            return Object.assign({}, state, { currentlyEditing: action.date })
...
export const handleAction = (state, action) => {
    if (! isSomething(state)) {
        ...
        return initialState(moment().utcOffset(), localStorage.getItem('credentials'), range, null)
    }
