...
import { isSomething, listToMap, nub, renderDate, renderDistance, renderDuration } from '../common'
import { cancelEditEntry, editEntry, runFetchHistory, runSaveTimeDistance, runSaveWeight } from '../state/actions'
...
class DailyEntry extends React.Component {
    ...
    render () {
        return  <div key={renderDate(this.props.data.date)}>
                    <h2> {renderDate(this.props.data.date)}
                         <button type="button" style={{float: 'right'}} className="btn btn-outline-primary" onClick={(ev) => this.props.onEdit(this.props.data.date)}>Edit</button>
                         </h2>
                    <div>
                        <div><WeightForm value={this.props.data.weight} /> </div>
                        <table className="table">
                            <tbody>
                                { isSomething(this.props.data)
                                    ? this.props.data.timeDistance.map(e => <TimeDistance data={e} />)
                                    : null }
                            </tbody>
                        </table>

                        <DailySummary tdEntries={this.props.data.timeDistance} />
                    </div>
                </div>
    }

    isTimeDistanceWorkout(s) { return s == "Cycling" || s == "Running" }
    isSetRepWorkout(s) { return s == "Crunches" || s == "Pushups" || s == "Situps" };
}

export const DailyEntryView =
    connect ( (st) => ({ })
...
class DailyEntryEdit extends React.Component {
    constructor (props) {
         ...
         */
        this.state = { weight: (isSomething(this.props.data) && isSomething(this.props.data.weight))
                                    ? this.props.data.weight.clone()
                                    : null
                                    // : weightSample(null, this.props.date, math.unit(0, 'kg'), math.unit(0, 'kg'))
                     , timeDistanceRows: (isSomething(this.props.data) && isSomething(this.props.data.timeDistance))
                                            ? this.props.data.timeDistance.map((e) => e.clone())
                                            : []
                     , newRow: null
                     , newRowWidget: null
                     }
    ...
    updateWeight (newVal) {
        if (isSomething(this.state.weight)) {
            this.setState({weight: Object.assign(this.state.weight, {weight: newVal})})
        } else {
            this.setState({weight: weightSample(null, this.props.date, newVal, math.unit(0, 'kg'))})
        }
    }
    ...
    render () {
        ...
        return (
            <div key={renderDate(this.props.date)}>
                ...
                <div> Weight: <WeightEditForm value={this.state.weight} onUpdate={(w) => this.updateWeight(w)} /> </div>
                <table className="table table-hover">
                    <tbody>
                    { this.state.timeDistanceRows.map(
                            (e) => <TimeDistanceEdit {...e}
                                                     onUpdateDate={(value) => e.date = value}
                                                     onUpdateDistance={(value) => e.distance = value}
                                                     onUpdateDuration={(value) => e.duration = value} />) }
                    </tbody>
                </table>
                <WorkoutPulldown onSelect={(option, evt) => this.setNewRow(option)} />
                {this.state.newRowWidget}
                <button type="button" className="btn btn-outline-primary" onClick={(ev) => this.props.onSave(this.props.data, this.state)}>Save</button>
                <button type="button" className="btn btn-outline-danger" onClick={(ev) => this.props.onCancel()}>Cancel</button>
            </div>)
    }

    setNewRow (option, evt) {
        if (option == 'Cycling' || option == 'Running') {
            var newRow = timeDistanceSample(null, moment(), option, math.unit(0, 'km'), math.unit(0, 's'))
            this.setState({newRow: newRow
                          , newRowWidget: <TimeDistanceEdit activity={option}
...
export const DailyEntryEditView =
    connect ( () => ({ })
            , (dispatch) => ({ onCancel: () => dispatch(cancelEditEntry())
...
const saveDailyEntryEdit = (dispatch, initialData, currentData) => {
    ...
    if (   (! isSomething(initialData) && isSomething(currentData.weight))
        || (isSomething(initialData) && ! isSomething(initialData.weight) && isSomething(currentData.weight)) 
    ...
    const initialTdrs = isSomething(initialData) && isSomething(initialData.timeDistance)
                            ? listToMap((e) => e.uuid, initialData.timeDistance)
                            : {}
    const tdrs = isSomething(currentData) && isSomething(currentData.timeDistanceRows)
...
const WorkoutPulldown = (props) => (
    <div key="workout-pulldown" className="dropdown">
        <button type="button" className="btn btn-outline-primary dropdown-toggle" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Add workout</button>
        <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
            <a className="dropdown-item" href="#" onClick={(e) => props.onSelect('Cycling', e)}>Cycling</a>
            <a className="dropdown-item" href="#" onClick={(e) => props.onSelect('Running', e)}>Running</a>
        </div>
    </div>
)
